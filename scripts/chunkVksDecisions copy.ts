 ...
 */
// ============================================================================
// ARGUMENT START PATTERNS
// ============================================================================

/**
 * Regexes that identify the first line of the ACTUAL ARGUMENT section — the
 * paragraph where the parties' positions are presented or the court begins its
 * substantive analysis.
 *
 * RATIONALE
 * ---------
 * `detectSections()` marks the header/body boundary at the first
 * BODY_START_PATTERNS match (e.g. "Производството е по чл.290 ГПК.").
 * However, that first paragraph — and sometimes the one that follows it
 * ("Образувано е по касационна жалба на X срещу решение Z на съд W...") —
 * is still CASE-IDENTITY / PROCEDURAL METADATA, not substantive reasoning:
 *
 *   • "Производството е по чл.290 ГПК."          ← procedure type (31 chars)
 *   • "Образувано е по касационна жалба на X..."  ← who filed, against what,
 *                                                     from which court (~1 000 chars)
 *
 * Treating these as reasoning chunks causes two problems:
 *   1. The 31-char boilerplate becomes a near-useless low-signal embedding that
 *      appears in ~1 100 / 1 114 decisions — identical across them all.
 *   2. The opener paragraph is mixed content (phrase + specific entities) → its
 *      embedding is diluted.  A substantive reasoning chunk about the cassation
 *      appeal naturally scores HIGHER for the query "Образувано е по касационна
 *      жалба" than the chunk that literally starts with that phrase, because the
 *      reasoning chunk is semantically denser in cassation-related concepts.
 *
 * FIX
 * ---
 * After detecting `bodyStartIdx`, `detectSections()` scans forward for the
 * first paragraph that matches ARGUMENT_START_PATTERNS and treats that as the
 * TRUE start of the reasoning section.  Everything between `bodyStartIdx` and
 * `argumentStartIdx` is included in the HEADER CHUNK as case metadata.
 *
 * Result:
 *   HEADER  = actTitle + formal header + procedure line + case opener paragraph(s)
 *   REASONING = starts at the first party-argument / court-analysis paragraph
 *
 * The cap MAX_OPENER_PARAGRAPHS (= 4) prevents pathological cases from
 * absorbing the entire body into the header when no ARGUMENT_START match fires.
 */
const ARGUMENT_START_PATTERNS: RegExp[] = [
  // ── Cassation appeal argument presentation ────────────────────────────────
  /^В касационната жалба/,
  /^С касационната жалба е въведено/,   // "С касационната жалба е въведено оплакване..."
  /^Касаторът (поддържа|твърди|сочи|навежда)/,
  /^Касаторите (правят|навеждат|твърдят|поддържат)/,   // plural form
  /^В жалбата (се (сочат|твърди|излага|навежда)|е посочено)/,
  /^Оплакванията (на касатора|в жалбата)/,
  /^Излагат се доводи/,

  // ── Admissibility / cassation selection ──────────────────────────────────
  /^Касационното основание/,
  /^Допуснато е касационно/,
  /^С определение.*допуснато/,

  // ── Чл.303 ГПК (molba za otmyana) argument presentation ─────────────────
  /^В молбата (за отмяна )?се (навеждат|правят|посочва|излагат)/,
  /^В молбата се посочва/,
  /^Молителят (прави|поддържа|твърди|навежда|сочи)/,

  // ── Court's own analysis intro ────────────────────────────────────────────
  /^Върховни[ят] касационен съд.*(взе предвид|за да се произнесе)/,
  /^ВКС.*за да се произнесе/,
  /^Настоящи[ят]\/ата.*съд.*взе предвид/,
  ...
  // ── Party positions ───────────────────────────────────────────────────────
  /^Жалбоподателят (поддържа|твърди)/,
  /^Ответникът по касация(та)? (твърди|поддържа|оспорва)/,
...
/**
 * Maximum number of body-start paragraphs to absorb into the header.
 * Guards against the (unlikely) case where ARGUMENT_START_PATTERNS never fires
 * and the entire body would otherwise be included in the header.
 */
const MAX_OPENER_PARAGRAPHS = 4;

function isArgumentStart(line: string): boolean {
  return ARGUMENT_START_PATTERNS.some(p => p.test(line.trim()));
}
...
interface Sections {
  ...
  flags: {
    ...
    noFooter:          boolean;
    /**
     * No ARGUMENT_START_PATTERNS match found within MAX_OPENER_PARAGRAPHS of
     * bodyStartIdx — case-opener paragraphs were absorbed into the header using
     * the fallback cap (up to MAX_OPENER_PARAGRAPHS paragraphs).
     */
    noArgumentStart:   boolean;
    /** Number of opener paragraphs absorbed into the header beyond bodyStartIdx. */
    openerParagraphsAbsorbed: number;
  };
}
...
interface Stats {
  ...
  noFooter:          number;
  noArgumentStart:   number;   // decisions where ARGUMENT_START_PATTERNS never matched
  openerParagraphsAbsorbed: number;  // total opener paragraphs moved into headers
...
function detectSections(actPlainText: string): Sections {
  ...
  // ── Step 1b: Extend header with case-opener paragraphs ───────────────────
  //
  // Paragraphs immediately after bodyStartIdx are often procedural metadata
  // ("Производството е по чл.290 ГПК.", "Образувано е по касационна жалба на X
  // срещу решение Z на съд W...") rather than substantive reasoning.  Including
  // them in the header produces a richer case-identity chunk and prevents
  // embedding dilution in the reasoning section.
  //
  // Scan forward from bodyStartIdx for the first paragraph that matches
  // ARGUMENT_START_PATTERNS (= real reasoning / party arguments start).
  // Everything before that becomes part of the header.
  //
  // Cap at MAX_OPENER_PARAGRAPHS to guard against missing patterns.
  let reasoningStartIdx = bodyStartIdx;  // default: no opener paragraphs absorbed
  let noArgumentStart   = true;
  const searchLimit = Math.min(bodyStartIdx + MAX_OPENER_PARAGRAPHS, paragraphs.length);

  for (let i = bodyStartIdx; i < searchLimit; i++) {
    const firstLine = paragraphs[i].split('\n')[0];
    if (isArgumentStart(firstLine)) {
      reasoningStartIdx = i;
      noArgumentStart   = false;
      break;
    }
  }

  // If no match found within the cap, keep reasoningStartIdx = bodyStartIdx
  // (original behaviour — no opener paragraphs absorbed).

  const openerAbsorbed = reasoningStartIdx - bodyStartIdx;

  const headerText    = paragraphs.slice(0, reasoningStartIdx).join('\n\n');
  const bodyAndRuling = paragraphs.slice(reasoningStartIdx).join('\n\n');
  ...
  if (rulingStartIdx === -1) {
    ...
    return {
      ...
      flags: {
        ...
        noFooter:                false,
        noArgumentStart,
        openerParagraphsAbsorbed: openerAbsorbed,
      },
    };
  }
  ...
  return {
    ...
    flags: {
      noBodyStartMatch,
      noRulingMarker:          false,
      noFooter:                footerIdx === -1,
      noArgumentStart,
      openerParagraphsAbsorbed: openerAbsorbed,
    },
  };
...
function chunkDecision(record: InputRecord, statsRef: Stats): Chunk[] {
  ...
  if (flags.noArgumentStart)  statsRef.noArgumentStart++;
  statsRef.openerParagraphsAbsorbed += flags.openerParagraphsAbsorbed;
...
function printStats(stats: Stats): void {
  ...
  console.log(`  No argument-start    (→ no opener pars absorbed)  : ${stats.noArgumentStart}`);
  console.log('');
  console.log(`Opener paragraphs absorbed into header : ${stats.openerParagraphsAbsorbed}`);
